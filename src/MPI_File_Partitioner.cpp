...
{	
    ...
    if (rank == MPI_Processes-1) end = filesize-1;
...
FileSplits* MPI_File_Partitioner :: partitionLayer(MPI_File mpi_layer, int block_size, MPI_Comm comm) {
	int nprocs, rank;
    ...
    /* except the last processor, of course */
    if (rank == nprocs-1) end = filesize-1;
    ...
    */
	...
	//printf("Rank,%d,read file at ,%ld, length at, %ld \n", rank, locstart, localsize);
    //fflush(stdout);    
    //cerr<<rank<<" 2nd : "<<locstart<<" , "<<locend<<endl;
    //printf("P%d, 2nd %lld : %lld\n", rank, locstart, locend);
    //fflush(stdout);
     ...
     for(int i = 0; i< localsize; i++)
   {
    ...
    data[localsize] = '\0';
    free(chunk);
	//printf("Rank %d, file length %lld : %lld\n", rank, sizeof(data));
    FileSplits *splits = new FileSplits();
    splits->WriteBuffer(data);
    splits->ChunkBuffer(block_size);
